...
###############################################
# 🔥 SAFE JSON LOADER ADDED (NEW CODE)
###############################################
def safe_json_loads(s: str):
    try:
        ...
        # Replace newlines inside JSON (common LLM issue)
        fixed = fixed.replace("\n", " ")
        ...
        try:
            ...
            return {}  # Return empty args to avoid crashing
            ...
            match message["type"]:
                ...
                case "response.output_item.done":
                    if "item" in message and message["item"]["type"] == "function_call":
                        ...
                        ###############################################
                        # ORIGINAL LINE (COMMENTED, YOUR REQUEST)
                        # result = await tool.target(json.loads(args))
                        ###############################################

                        ###############################################
                        # REPLACED WITH SAFE JSON PARSING
                        ###############################################
